...
export function AppShell({ children, organization = "Nexus Digital", demo = false }: { children: React.ReactNode; organization?: string; demo?: boolean }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <div className="lg:pl-64">
      ...
      <main className="p-4 sm:p-7">{demo && <div className="mb-5 flex items-start gap-3 rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-sm"><Activity className="mt-0.5 size-4 text-amber-300" /><div><strong>Banco de dados não configurado.</strong><p className="text-muted-foreground">As operações estão indisponíveis e nenhum dado fictício é exibido.</p></div></div>}{children}</main>
    </div>
  </div>;
}
