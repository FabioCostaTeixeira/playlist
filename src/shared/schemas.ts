...
function validateContent(
  value: Partial<z.infer<typeof contentFields>>,
  ctx: z.RefinementCtx,
) {
  if (value.type === "url") {
  ...
  if (value.type && ["image", "video"].includes(value.type) && !value.blobPath)
    ctx.addIssue({
      code: "custom",
      path: ["blobPath"],
      message: "Arquivo obrigatório",
    });
  if (value.type === "html" && !value.html)
    ctx.addIssue({
      code: "custom",
      path: ["html"],
      message: "HTML obrigatório",
    });
}

export const contentInput = contentFields.superRefine(validateContent);
export const contentPatchInput = contentFields
  .partial()
...
export const deviceInput = z.object({
  ...
  description: z.string().trim().max(500).optional().nullable(),
  status: z
    .enum(["pending", "active", "blocked", "archived"])
    .default("pending"),
...
export const scheduleInput = scheduleFields
  .extend({
    priority: scheduleFields.shape.priority.default(0),
    status: scheduleFields.shape.status.default("active"),
  })
  .refine((value) => !value.endsAt || value.endsAt > value.startsAt, {
    path: ["endsAt"],
    message: "Término deve ser posterior ao início",
  });
export const schedulePatchInput = scheduleFields
  .partial()
  .refine(
    (value) =>
      !value.endsAt || !value.startsAt || value.endsAt > value.startsAt,
    { path: ["endsAt"], message: "Término deve ser posterior ao início" },
  );
...
export const emergencyInput = z
  ...
  .refine(
    (value) => Boolean(value.contentId) !== Boolean(value.playlistId),
    "Informe conteúdo ou playlist, nunca ambos",
  )
  .refine((value) => value.endsAt > new Date(), {
    path: ["endsAt"],
    message: "Expiração obrigatoriamente futura",
  });

export const pairingInput = z.object({ code: z.string().regex(/^\d{6}$/) });
export const heartbeatInput = z.object({
  resolutionWidth: z.number().int().positive().max(16384).optional(),
  resolutionHeight: z.number().int().positive().max(16384).optional(),
  playerVersion: z.string().max(40).optional(),
});
...
export type ResourceName =
  "contents" | "playlists" | "channels" | "devices" | "schedules";
