...
export async function PATCH(
  ...
  return api(async () => {
    ...
    if (id === actor.userId && value.status === "blocked")
      throw new Response("Não bloqueie a própria conta", { status: 409 });
    ...
    await audit(actor, "member_updated", "user", id, undefined, value);
    return row;
  });
}
...
export async function DELETE(
  ...
  return api(async () => {
    ...
    const { id } = await context.params;
    if (id === actor.userId)
      throw new Response("Não remova a própria conta", { status: 409 });
    const [row] = await getDb()
    ...
    if (!row) throw new Response("Não encontrado", { status: 404 });
    await audit(actor, "member_removed", "user", id);
    return { ok: true };
  });
}
