    ...
    .array(
      z.object({
        ...
        durationSeconds: z
          ...
          .nullable()
          .optional(),
        transitionType: z.enum(["none", "fade"]).default("fade"),
...
export async function PUT(
  ...
  return api(async () => {
      ...
      const owned = await getDb()
        ...
        .where(
          and(
            ...
            inArray(contents.id, contentIds),
          ),
        );
      if (new Set(owned.map((row) => row.id)).size !== new Set(contentIds).size)
        throw new Response("Conteúdo de outro tenant ou inexistente", {
          status: 422,
        });
    }
    await getDb().delete(playlistItems).where(eq(playlistItems.playlistId, id));
    ...
    await audit(actor, "reorder_items", "playlist", id, undefined, {
